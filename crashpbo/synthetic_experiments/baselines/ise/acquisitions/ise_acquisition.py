...
from ..utils.optimization_utils import IseConstrainedOptimizer
from ..utils.generic_utils import sample_uniform_in_box
...
DEFAULT_ISE_STARTER_TIMEOUT_SECONDS = _get_env_float("ISE_STARTER_TIMEOUT_SECONDS", 120.0)
DEFAULT_ISE_STARTER_MAX_ROUNDS = _get_env_int("ISE_STARTER_MAX_ROUNDS", 400)
...
class IseAcquisition:
    def __init__(
            ...
            noise_variance=None,
            *,
            starter_timeout_seconds=None,
            max_starter_sampling_rounds=None):
        '''
        Constructor
        ...
        starter_timeout_seconds (float or None): Optional wall-clock time budget for sampling safe starting
        points (defaults to env ISE_STARTER_TIMEOUT_SECONDS or 120s)
        max_starter_sampling_rounds (int or None): Optional cap on how many sampling rounds are attempted before
        falling back to the safe seed (defaults to env ISE_STARTER_MAX_ROUNDS or 400)
        '''
        ...
        self._starter_timeout_seconds = (
            starter_timeout_seconds
            if starter_timeout_seconds is not None else DEFAULT_ISE_STARTER_TIMEOUT_SECONDS
        )
        self._max_starter_sampling_rounds = (
            max_starter_sampling_rounds
            if max_starter_sampling_rounds is not None else DEFAULT_ISE_STARTER_MAX_ROUNDS
        )
    ...
    def _safe_seed_batch(self, number_of_points):
        '''
        Replicates the first safe seed to match the batch shape required by the optimizer.
        '''
        safe_seed = self._safe_seed
        if safe_seed.dim() == 1:
            safe_seed = safe_seed.unsqueeze(0)
        if safe_seed.dim() == 2:
            safe_seed = safe_seed.unsqueeze(1)
        return safe_seed[:1].repeat(number_of_points, 1, 1)

    def _sampling_limits_reached(self, sampling_round, start_time):
        '''
        Determine whether either the timeout or the sampling round limit has been hit.
        '''
        timed_out = (
            self._starter_timeout_seconds is not None
            and (time.monotonic() - start_time) > self._starter_timeout_seconds
        )
        limit_hit = (
            self._max_starter_sampling_rounds is not None
            and sampling_round >= self._max_starter_sampling_rounds
        )
        return timed_out, limit_hit
    ...
    def _select_optimization_starting_points(self, number_of_staring_points):
        ...
        while True:
            ...
            if self._points_are_safe(starters_x):
                return starters_x.detach(), potential_starters_z[max_values_indices].detach()

            sampling_round += 1
            timed_out, limit_hit = self._sampling_limits_reached(sampling_round, start_time)
            if timed_out or limit_hit:
                ...
                warnings.warn(
                    "ISE acquisition could not find enough safe starting points before hitting the "
                    f"{' and '.join(reason)}; falling back to evaluating the safe seed."
                )
                safe_batch = self._safe_seed_batch(number_of_staring_points).detach()
                return safe_batch, safe_batch.clone().detach()
