...
DEFAULT_ISE_LINE_BO_TIMEOUT_SECONDS = _get_env_float("ISE_LINE_BO_TIMEOUT_SECONDS", 120.0)
DEFAULT_ISE_LINE_BO_MAX_ROUNDS = _get_env_int("ISE_LINE_BO_MAX_ROUNDS", 400)
...
class IseLineBoAcquisition(LineBoAcquisitionBase):
    def __init__(
        ...
        timeout_seconds (float or None): maximum wall-clock time for the internal line sampling loop (defaults to
            env ISE_LINE_BO_TIMEOUT_SECONDS or 120s)
        max_sampling_rounds (int or None): hard limit on sampling rounds before giving up (defaults to env
            ISE_LINE_BO_MAX_ROUNDS or 400)
        '''

        timeout_seconds = (
            timeout_seconds if timeout_seconds is not None else DEFAULT_ISE_LINE_BO_TIMEOUT_SECONDS
        )
        max_sampling_rounds = (
            max_sampling_rounds if max_sampling_rounds is not None else DEFAULT_ISE_LINE_BO_MAX_ROUNDS
        )
        super().__init__(
            safe_seed,
            domain,
